...
// TODO /////////////////////////////////////
export const AlertPolygon = ({ feature, color }) => {
	...
	const d3PathGen = d3.geoPath(albersFitExtent);

	return (
		<AlertCardSubComponent>
			<USCountyMap pathGen={d3PathGen}>
				<path
					d={d3PathGen(TurfRewind(feature, { reverse: true }))}
					fill={color}
					stroke={`dark${color}`}
					strokeWidth={10}
					opacity={0.5}
		...
		<AlertCardSubComponent className='mb-2'>
			{impactedAreasMapEntries
				? impactedAreasMapEntries.map(([state, areas]) => {
						...
						return (
							<div key={`${state}-${areas.slice(0, 10)}`}>
								{state ? (
