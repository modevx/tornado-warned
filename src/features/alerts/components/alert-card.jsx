...
import { isDestructiveStorm, isPdsStorm, isTornadoEmergency, isWarningEvent } from "utils/nws-alerts";
import { ALERT_TAGS } from "constants/nws-alerts";
import { ALERT_COLORS } from "styles/nws-alert-colors";
import { AlertPolygon, WarningPolygon, WatchPolygon } from "components/nws_alerts/AlertPolygons";
import { albersCountiesGeoJson, albersStatesGeoJson, albersProjection, albersGeoPath, createWatchAlertGeometry } from "utils/geometry";
...
export const ActiveAlertCard = ({ alert, showAlertModalFunc }) => {
  const {
    ...
    instruction,
    senderName,
    parameters: { maxHailSize, maxWindGust, tornadoDetection, thunderstormDamageThreat },
  } = alert?.properties;
...
const SituationTag = ({ situation, color }) => {
  return (
    <>
      {situation && (
        <div style={{ backgroundColor: color }} className="text-center font-bold uppercase p-2 rounded">
          {situation}
        </div>
      )}
    </>
